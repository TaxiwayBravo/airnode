...
function setupCoordinatePicker(){if($('coordinate-picker'))return;const lat=$('latitude'),lon=$('longitude');if(!lat||!lon)return;const b=document.createElement('button');b.id='coordinate-picker';b.type='button';b.textContent='Use my location';b.title='Fill the receiver coordinates from this device';b.addEventListener('click',()=>{if(!navigator.geolocation){notice('Location is unavailable in this browser.',true);return}b.disabled=true;navigator.geolocation.getCurrentPosition(pos=>{lat.value=pos.coords.latitude.toFixed(6);lon.value=pos.coords.longitude.toFixed(6);b.disabled=false;notice('Receiver coordinates filled from your device location. Save receiver settings to apply them.');},()=>{b.disabled=false;notice('Location permission was not granted. Enter coordinates manually.',true);},{enableHighAccuracy:false,timeout:10000,maximumAge:300000});});lon.parentElement?.after(b);}
...
async function updateAircraftMap(){const box=ensureMapContainer();if(!box)return;const positions=(aircraft&&!aircraft.stale?aircraft.aircraft:[]).filter(a=>Number.isFinite(a.lat)&&Number.isFinite(a.lon)&&a.seen_pos<60);const center=config?.receiver;try{const L=await loadLeaflet();if(!map){const lat=Number.isFinite(center?.latitude)?center.latitude:(positions[0]?.lat||51.5),lon=Number.isFinite(center?.longitude)?center.longitude:(positions[0]?.lon||-0.1);map=L.map(box,{worldCopyJump:true}).setView([lat,lon],Number.isFinite(center?.latitude)?8:4);L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png',{maxZoom:19,attribution:'© OpenStreetMap contributors © CARTO'}).addTo(map);const msg=box.querySelector('.map-message');if(msg)msg.remove();}const seen=new Set;positions.forEach(a=>{seen.add(a.hex);let marker=mapMarkers.get(a.hex);if(!marker){marker=L.circleMarker([a.lat,a.lon],{radius:7,color:'#fff',weight:2,fillColor:'#117f79',fillOpacity:.95}).addTo(map);mapMarkers.set(a.hex,marker);}marker.setLatLng([a.lat,a.lon]).bindPopup('<strong>'+escapeHTML((a.flight||a.hex).trim())+'</strong><br>'+escapeHTML(a.desc||a.t||'Aircraft type unavailable')+'<br>'+escapeHTML(a.alt_baro==null?'Altitude unavailable':String(a.alt_baro)+' ft'));});mapMarkers.forEach((marker,key)=>{if(!seen.has(key)){map.removeLayer(marker);mapMarkers.delete(key);}});if(!positions.length){const msg=box.querySelector('.map-message');if(msg)msg.textContent=aircraft?.stale?'Receiver data unavailable or stale.':'Map ready. Waiting for fresh aircraft positions.';}}catch{const msg=box.querySelector('.map-message');if(msg)msg.textContent='Map tiles are unavailable. Check the Pi internet connection or enter coordinates manually.';}}
...
async function boot() {
  try {
    ...
    if(session.authenticated){setupTheme();setupCoordinatePicker();}
