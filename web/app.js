...
async function updateAircraftMap(){const box=ensureMapContainer();if(!box)return;const positions=(aircraft&&!aircraft.stale?aircraft.aircraft:[]).filter(a=>Number.isFinite(a.lat)&&Number.isFinite(a.lon)&&a.seen_pos<60);if(!positions.length){const msg=box.querySelector('.map-message');if(msg)msg.textContent=aircraft?.stale?'Receiver data unavailable or stale.':'Waiting for fresh aircraft positions.';return;}try{const L=await loadLeaflet();if(!map){map=L.map(box,{worldCopyJump:true}).setView([positions[0].lat,positions[0].lon],7);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:18,attribution:'© OpenStreetMap contributors'}).addTo(map);const msg=box.querySelector('.map-message');if(msg)msg.remove();}const seen=new Set;positions.forEach(a=>{seen.add(a.hex);let marker=mapMarkers.get(a.hex);if(!marker){marker=L.circleMarker([a.lat,a.lon],{radius:7,color:'#fff',weight:2,fillColor:'#117f79',fillOpacity:.95}).addTo(map);mapMarkers.set(a.hex,marker);}marker.setLatLng([a.lat,a.lon]).bindPopup('<strong>'+escapeHTML((a.flight||a.hex).trim())+'</strong><br>'+escapeHTML(a.alt_baro==null?'Altitude unavailable':String(a.alt_baro)+' ft')+' · '+escapeHTML(a.seen)+'s ago');});mapMarkers.forEach((marker,key)=>{if(!seen.has(key)){map.removeLayer(marker);mapMarkers.delete(key);}});if(!map._airnodeFit){map.fitBounds(L.latLngBounds(positions.map(a=>[a.lat,a.lon])),{padding:[30,30],maxZoom:9});map._airnodeFit=true;}}catch{const msg=box.querySelector('.map-message');if(msg)msg.textContent='Map tiles are unavailable. Use the aircraft links below or connect the Pi to the internet.';}}
...
function rows(list,map=false){
  return list.map(a=>`<tr><td><span class="plane-icon">✈</span><strong>${escapeHTML((a.flight||'').trim()||a.hex)}</strong><small>${escapeHTML(a.hex)}</small></td><td>${typeof a.alt_baro==='number'?a.alt_baro.toLocaleString()+' ft':escapeHTML(a.alt_baro)}</td><td>${a.gs==null?'—':escapeHTML(Math.round(a.gs))+' kt'}</td><td>${a.rssi==null?'—':escapeHTML(a.rssi)+' dBFS'}</td><td>${escapeHTML(a.seen)}s</td>${map?`<td>${Number.isFinite(a.lat)&&Number.isFinite(a.lon)&&a.seen_pos<60?`<a target="_blank" rel="noopener noreferrer" href="https://www.openstreetmap.org/?mlat=${encodeURIComponent(a.lat)}&mlon=${encodeURIComponent(a.lon)}#map=9/${encodeURIComponent(a.lat)}/${encodeURIComponent(a.lon)}">Map ↗</a>`:'—'}</td>`:''}</tr>`).join('')||`<tr><td class="empty" colspan="${map?6:5}">${aircraft?.stale?'Receiver data unavailable or stale.':'No aircraft heard yet. Give your antenna a clear view of the sky.'}</td></tr>`;
}
