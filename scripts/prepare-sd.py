...
config = {'ssid': 'AirNode-Setup-' + secrets.token_hex(3).upper(), 'password': 'unused-open-setup', 'country': sys.argv[2]}
token = secrets.token_urlsafe(24)
paths[0].write_text(json.dumps(config), encoding='utf-8')
paths[1].write_text(token, encoding='utf-8')
paths[2].write_text('AirNode setup details. The setup Wi-Fi is open and local-only.\nWi-Fi: ' + config['ssid'] + '\nWi-Fi password: none\nOpen: https://10.42.0.1/\nPairing token: ' + token + '\n', encoding='utf-8')
